namespace Year2021
{
	public class Day17Solver
	{
		...
		public long FindHighestProbePositionWhenLaunchedWithStyle()
		{
			...
			// completely based upon it touching the target and not missing the target after one single step after its return from origin.
			...
			{
				var bottomY = Math.Min(Target.Y1, Target.Y2);
				var velocity = Math.Abs(bottomY) - 1;
				long y = 0;
				while (velocity > 0)
				{
					y += velocity;
			...
			if (Target.Y1 > 0 && Target.Y2 > 0)
			{
				var topY = Math.Max(Target.Y1, Target.Y2);
				var velocity = topY;
				long y = 0;
				while (velocity > 0)
				{
		...
		public IEnumerable<(long, long)> FindInitialVelocitiesThatHitTarget()
		{
			...
			// spans the x origin, then the min and max xvel loop boundaries will both be target edges. If the target exists entirely to the
			...
			// the way to the right most edge of the target to find the velocities.
			var xvel1 = Math.Min(0, Math.Min(Target.X1, Target.X2));
			var xvel2 = Math.Max(0, Math.Max(Target.X1, Target.X2));
