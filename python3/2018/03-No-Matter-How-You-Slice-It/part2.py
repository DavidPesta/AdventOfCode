...
list_of_claims_without_overlaps = []

for claim in claims:
	parsed_claim = parse_claim(claim)
	
	overlap_found_in_this_claim = False
	for y in range(parsed_claim["StartY"], parsed_claim["EndY"]):
		for x in range(parsed_claim["StartX"], parsed_claim["EndX"]):
			if whole_fabric[x][y] == "X":
				overlap_found_in_this_claim = True
	
	if overlap_found_in_this_claim == False:
		list_of_claims_without_overlaps.append(parsed_claim["ID"])

print(list_of_claims_without_overlaps[0])